...
class YoloTRT():
    def __init__(self, library, engine, conf, yolo_ver):
        ...
        self.categories = ["lane", "stop", "spd100", "spd50", "traffic", "park", "aves", "feline", "ungulates","parking_area"]
    ...
    def detect(self, img: np.array):
        ...
        for j in range(len(result_boxes)):
            ...
            det["clsid"] = cls_id
            det["class"] = self.categories[cls_id]
            det["conf"] = result_scores[j]
            det["box"] = box
            det_res.append(det)
            # self.PlotBbox(det["center"], box, img, label="{}:{:.2f}".format(self.categories[int(result_classid[j])], result_scores[j]), )
        return det_res
    ...
    def NonMaxSuppression(self, prediction, origin_h, origin_w, conf_thres=0.5, nms_thres=0.6):
